...
### Preset E — "Strategic Ink" (Creative Studio)
- **Identity:** Um estúdio de direção criativa orientado por estratégia encontra um sistema editorial contemporâneo; entre estrutura e fluidez, a marca evolui com intenção para comunicar e posicionar.
- **Palette:** Preto Profundo `#0D0D0D` (Primary / Background), Bege Suave `#FAF6DF` (Text), uso mínimo de variação cromática com foco em contraste e respiro.
- **Typography:** Headings: "Gilroy Black". Drama: "Gilroy Black Italic". Body: "Gilroy Regular". Accent: "DeFont Reduced".
- **Image Mood:** editorial minimalista com atitude, fundos escuros, composições tipográficas e elementos orgânicos como nanquim e blur (pesquise: experimental typography, editorial black layout, ink textures, brutalist editorial).
- **Padrão de frase do Hero:** "[Estado inicial] ao [estado final]." (Bold Sans) / "[Palavra de transformação]." (Italic)
